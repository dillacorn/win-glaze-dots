...
- YASB Quick Launch is the primary Awtarchy-style application search, with installed applications as the default provider. YASB owns one private `F24` global hotkey; GlazeWM owns the user-facing launcher chords and invokes the dotfile-owned `~/.config/win-glaze/scripts/yasb-quick-launch.ps1` bridge. This preserves VM pass-through because YASB must not globally register `Alt+P` or `Win+D`. The Normal profile maps `Alt+P` plus `Win+D` to that standalone YASB bridge. On `feat/restricted-work-pc`, the Work profile intentionally A/B tests launchers for this migration only: `Alt+P` invokes the standalone Flow Launcher script while `Win+D` invokes the YASB bridge, including noalt mode. Do not generalize that Work-profile split into the normal project default or make Flow Launcher default-on without a fresh explicit decision. Flow Launcher otherwise remains optional/default-off.
