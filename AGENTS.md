...
- EarTrumpet owns `Alt+V` through its own application hotkey setting. GlazeWM must not capture `Alt+V` or `Super+V`, and WGDot must not rewrite EarTrumpet's mixer hotkey during software reconciliation. YASB volume right-click may activate EarTrumpet directly through `explorer.exe shell:AppsFolder\40459File-New-Project.EarTrumpet_725pr5jq8wr8a!EarTrumpet`; keep exactly one backslash after `AppsFolder` and do not route this through WGDot. Native Windows `Super+V` stays free for Clipboard History.
...
- Keep native YASB volume left-click mute, middle alternate-label behavior, and wheel volume changes. Volume right-click directly activates the EarTrumpet packaged app through the single-separator AppsFolder target; the retired doubled-backslash target that opened Explorer/Documents must not return.
