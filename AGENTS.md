...
- WGDot runtime helpers are permitted for narrowly scoped custom primitives that native owners cannot provide cleanly, including coordinated YASB/GlazeWM auto-hide, idle inhibition through Windows execution-state APIs, live Awtarchy-style theme application/synchronization and selector-window toggling, the Awtarchy-style power surface, the Awtarchy-style application launcher, the experimental native Clipboard History anchor, and the low-level mouse-button hook GlazeWM cannot express. Do not turn WGDot into a general desktop broker.
