...
- Preserve WGDot's existing reload-free reservation model on this branch: YASB is `always_on_top: true`, `windows_app_bar: false`, and both GlazeWM profiles keep the 38 px top `outer_gap`. Migrating a live session to AppBar reservation would require a GlazeWM config reload to adopt a smaller gap; do not force that layout-disrupting reload merely for the bar redesign.
...
- Use YASB's supported `yasbc toggle-bar` for WGDot's legacy `Alt+Ctrl+B` hard-visibility convenience; never kill/restart the YASB process merely to hide the bar. Do **not** map Awtarchy's `Win+Alt+Ctrl+B` auto-hide chord to `toggle-bar`: the command hides YASB but cannot reclaim WGDot's separate 38 px GlazeWM gap. YASB has native `auto_hide`, but v2.0.7 exposes no supported runtime CLI toggle for that configuration flag. Keep `auto_hide: false` explicit until a direct focused-monitor mapping exists.
