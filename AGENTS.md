...
- WGDot contains a reversible, default-off `disable-windows-shell-hotkeys` tweak for Microsoft's per-user `NoWinKeys` Explorer policy. It snapshots/restores `HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\NoWinKeys`. Real Windows testing on 2026-09-20 confirmed `NoWinKeys=1` allows the tested GlazeWM Super namespace after sign-out/sign-in. Creating the protected policy key required UAC on that machine, so direct tweak commands must retain their access-denied elevation fallback. Keep the tweak default-off unless the project explicitly changes that choice.
...
- YASB Quick Launch is WGDot's primary Awtarchy-style application search, with installed applications as the default provider. Upstream YASB v2.0.7 implements widget hotkeys with Win32 `RegisterHotKey`, so a YASB-owned `Alt+P` or `Win+D` would remain global even while GlazeWM is in VM mode. Keep only VM-safe `Win+Alt+D` registered by YASB. Normal/noalt GlazeWM own `Alt+P` and `Win+D` and route them through `wgdot quick-launch`, which waits for the physical modifiers to release, preserves GlazeWM's current pause state, pauses GlazeWM while injecting the one YASB-owned `Win+Alt+D` chord so GlazeWM's low-level hook cannot consume the synthetic Windows key, then restores the prior pause state. Do not bind `Win+Alt+D` to the WGDot helper or it can recurse. Flow Launcher remains an optional legacy package/helper and its `Alt+P` tweak defaults off so it cannot collide with Quick Launch.
...
- Awtarchy's clipboard button maps to Windows Clipboard History through WGDot's helper and the bar's simple CustomWidget; keep Quick Launch's clipboard provider disabled so it does not duplicate that surface.
- Use YASB's native `ControlCenterWidget` for Awtarchy-style quick settings. Keep brightness/volume/microphone/media on native YASB sections and expose WGDot quick actions for themes, running-app visibility, themed running-app slot shading, coordinated bar auto-hide, mouse mode, and Windows display settings. Stock YASB does not expose Awtarchy's per-icon colorization; `Shade Apps` is explicitly a themed background-state approximation, not image tinting.
...
- YASB v2.0.7 Grouper is click-only. Do not use it for the workspace mover. Keep the mouse hub and directional workspace controls in one Applications widget, clamp the widget to the hub at rest, and use native QSS `:hover` width expansion so the arrows reveal without a click.
