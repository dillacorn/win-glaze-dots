...
- Native ownership must stay literal for actions that have a clean native surface: screenshots, Windows Settings, EarTrumpet, workspace movement, and GlazeWM pause/mode state must not be bounced through WGDot merely for convenience. The application launcher is an explicit exception because upstream YASB cannot provide the required trigger-aware placement or a native CLI/widget-action interface.
...
- WGDot runtime helpers are permitted for narrowly scoped custom primitives that native owners cannot provide cleanly, including coordinated YASB/GlazeWM auto-hide, idle inhibition through Windows execution-state APIs, live Awtarchy-style theme application/synchronization and selector-window toggling, the Awtarchy-style power surface, the Awtarchy-style application launcher, and the experimental native Clipboard History handoff. The retired mouse-mode low-level pointer hook is not an approved runtime primitive. Do not turn WGDot into a general desktop broker.
...
- Launcher hotkeys are GlazeWM-owned but dispatch the approved compiled `wgdotw.exe launcher hotkey` surface because current upstream YASB Quick Launch cannot distinguish bar-click placement from keyboard placement and `yasbc` exposes no widget-action command. In managed GlazeWM config, invoke the helper through its quoted `%LOCALAPPDATA%\wgdot\bin\wgdotw.exe` path so a long-running GlazeWM process does not depend on a stale inherited PATH. Do not restore `flow-launcher.ps1`, `yasb-quick-launch.ps1`, SendKeys, a private/synthetic F24 relay, or any fake YASB key injection. Global `Alt+P` and `Super+D` open the compiled launcher; `noalt` keeps `Super+D` but deliberately leaves plain `Alt+P` uncaptured. Flow Launcher remains optional/default-off unless the user explicitly chooses it.
- Target Awtarchy-style clipboard/audio bindings without routing EarTrumpet through WGDot. Native Windows Clipboard History is working again on the gaming PC, but it still has no documented public API for opening/positioning the shell surface. The only approved exception is the narrow experimental `wgdotw.exe clipboard-anchor [bar|hotkey]` helper: reliability takes precedence over synthetic placement, so it must preserve the user's foreground context, wait for modifier release, preserve/temporarily verify GlazeWM's native pause state solely around injected `Win+V`, restore that state, and exit. Do not create a temporary anchor window merely to fake bar-relative placement, and do not revive the retired generic `clipboard-history` worker/window.
- Live audio ownership is GlazeWM -> EarTrumpet directly: global `Super+V` and `Alt+V` launch the current packaged AppsFolder identity `40459File-New-Project.EarTrumpet_1sdd7yawvg6ne!EarTrumpet`, while `noalt` retains `Super+V` but deliberately leaves plain `Alt+V` uncaptured. WGDot's EarTrumpet packaged-setting integration may remain only as a management-time fallback/configuration path; desktop use must not route mixer activation through a WGDot synthetic-key helper.
- Flameshot selection capture is `Win+Shift+S` in both managed profiles and launches `flameshot.exe gui` directly from GlazeWM. WGDot must not sit in the live screenshot path. The tracked Flameshot INI must remain portable: no user-specific absolute profile paths and no obsolete shortcut entries that current Flameshot rejects. Keep the manifest file merge-safe so valid user settings are preserved while managed upstream defaults stop reintroducing invalid values.
...
- Keep the Windows bar on the managed `JetBrainsMono NFP` 14 px family for now. It is the installed Nerd Font with verified glyph coverage and is the supported similar-font match to Awtarchy's `NotoSansM Nerd Font Mono`. Do not reference the exact Awtarchy Noto family in Windows CSS unless WGDot first gains a verified managed Windows installation path for that family.
...
- Awtarchy's bar idle inhibitor uses `SetThreadExecutionState(ES_CONTINUOUS | ES_SYSTEM_REQUIRED | ES_DISPLAY_REQUIRED)` and clears the request on exit. Both profiles use the approved compiled WGDot idle helper. YASB v2.0.7 has no public targeted per-widget refresh IPC, so keep the normal CustomWidget status poll slow (30 seconds) and have an explicit toggle request one silent public `yasbc reload -s` so every monitor/bar immediately re-queries the new state. Do not alter the Windows power plan, tighten polling into a busy refresh loop, or invoke `idle-inhibitor.ps1` at runtime.
...
- Launcher placement is trigger/context aware: a visible-bar click opens directly below the top bar at the top-left of the active display; ordinary keyboard activation opens centered directly below the top bar; YASB auto-hide or a fullscreen/borderless foreground window forces center-screen placement on the relevant monitor. Keep the launcher single-instance and windowless at dispatch. Start Menu shortcut activation must use Windows shell execution, and launcher results must render shell-derived application icons.
