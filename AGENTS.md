...
- Restricted Work-PC operation must assume `.ps1` files can be unreadable as well as unexecutable. The Work GlazeWM/YASB profiles must therefore contain zero runtime references to `.ps1` files, and the `desktop-scripts` manifest component must default off for Work. Inline `powershell.exe -Command` is allowed only for genuine custom behavior that has no native Windows/YASB/GlazeWM interface, must not load a script file, must not use `-ExecutionPolicy Bypass`, and must not be used as a relay for ordinary launcher/keybinding behavior.
...
- Theme switching must not reload GlazeWM. Keep the focused-window border theme-neutral at `#a1a1a1`. In the Normal profile, `Win+T` launches the portable non-elevated `~/.config/win-glaze/scripts/theme-switcher.ps1` in Windows Terminal and that script owns YASB palette changes plus Windows Terminal synchronization. In the restricted Work profile, do not reference that script; `Win+T` opens native Windows Themes settings and the managed YASB palette remains static unless a future native/scriptless theme interface is added.
...
- Awtarchy's bar idle inhibitor uses `SetThreadExecutionState(ES_CONTINUOUS | ES_SYSTEM_REQUIRED | ES_DISPLAY_REQUIRED)` and clears the request on exit. The Normal profile may implement that through the portable `idle-inhibitor.ps1`. The restricted Work profile must implement the same behavior with inline PowerShell commands owned by the YASB widget, with no `.ps1` file dependency. WGDot does not own or restore the worker. Do not alter the user's Windows power plan or claim Awtarchy's separate four-hour Hypridle safeguard exists on Windows.
...
- Do not use `yasbc toggle-bar` for bar hiding: it can strand an invisible bar and does not coordinate the GlazeWM top gap. Managed config starts visible with YASB `auto_hide: false` and a 35 px GlazeWM top gap. The Normal profile may use `bar-autohide.ps1`; the restricted Work profile must perform the same config edit/reload logic inline with `powershell.exe -Command`, never by loading a `.ps1` file. Keep YASB `context_menu: false`; WGDot must not be required for bar auto-hide.
...
- In the Normal profile, keep quick-settings theme launch and `Win+T` on the same Windows Terminal `theme-switcher.ps1` selector. In the restricted Work profile, both custom theme-script entry points are absent; use native Windows Themes/Dark Mode surfaces instead. Do not reintroduce a `wgdot theme` runtime command or WGDot-owned active-theme state.
- In the Normal profile, launch the standalone theme selector in Windows Terminal titled `Win Glaze Themes` with `--suppressApplicationTitle` and keep its matching GlazeWM `set-floating --centered` rule. The restricted Work profile must not contain that title rule or script launch because the selector is unavailable there. Normal-profile theme application is owned by `~/.config/win-glaze/scripts/theme-switcher.ps1`, updates the tracked YASB theme file and Windows Terminal user settings, and must not reload GlazeWM.
