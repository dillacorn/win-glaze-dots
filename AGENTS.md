...
- Runtime ownership must stay literal, not approximate: GlazeWM keybindings should contain direct GlazeWM commands, direct executable launches, or documented native application/YASB interfaces. A helper that exists only to locate an executable, emit another key, or bounce an action into YASB is prohibited for normal desktop bindings.
- Restricted Work-PC operation must assume downloaded `.ps1` runtime helpers may be blocked. Core navigation, launcher binds, binding modes, pause, screenshots, and normal YASB bar interaction must therefore not depend on executing PowerShell script files. If an optional feature genuinely requires a script and no native supported interface exists, treat that feature as optional/unsupported on the restricted profile rather than disguising the dependency behind another launcher.
- Custom desktop behavior that genuinely needs logic may live in ordinary non-elevated scripts shipped with the dots. WGDot may install/update those scripts, but must not be required each time the user presses a key or clicks a widget. Do not use a PowerShell/helper script merely to relay a GlazeWM keybinding to an application, YASB action, synthetic keypress, or other behavior that GlazeWM/YASB/the target application can own directly.
...
- The managed profiles include `noalt`, `mouse`, and `vm` binding modes owned entirely by GlazeWM. `Win+Alt+N`, `Win+Alt+M`, and `Win+Alt+V` enter the corresponding mode; the same mode chord disables that mode, and mode-to-mode transitions explicitly disable the current mode before enabling the next one. Ordinary Windows/Alt chords are intentionally not captured in VM mode so a focused guest can receive them. YASB's native `GlazewmBindingModeWidget` displays and clears the active mode. Do not reintroduce WGDot-tracked binding-mode state or route mode transitions through WGDot.
...
- Windows RawAccel uses only `Win+Shift+M` / `Super+Shift+M`. Do not add the Awtarchy `Alt+Shift+M` alias on Windows. GlazeWM invokes the portable non-elevated `~/.config/win-glaze/scripts/rawaccel-toggle.ps1`, which closes an existing RawAccel GUI or resolves `rawaccel.exe` from PATH and standard install locations before launching it. WGDot must not be required for this runtime toggle.
- Theme switching must not reload GlazeWM. Keep the focused-window border theme-neutral at `#a1a1a1`; `Win+T` launches the portable non-elevated `~/.config/win-glaze/scripts/theme-switcher.ps1` in Windows Terminal. The script owns YASB palette changes and Windows Terminal synchronization; WGDot is not part of the live theme path.
- GlazeWM 3.10.x does not expose mouse buttons through its keybinding parser. The managed `mouse` binding mode is therefore a native GlazeWM mode/state only: `Win+Alt+M` must toggle it on/off directly and YASB may display that state, but do not claim it provides mouse-button move/resize behavior. Any future pointer hook must be separately designed as a portable non-WGDot implementation and must not be smuggled in merely to make the mode name functional.
...
- Awtarchy's bar idle inhibitor maps to the portable non-elevated `~/.config/win-glaze/scripts/idle-inhibitor.ps1`. YASB polls/toggles that script; its hidden user-session worker calls `SetThreadExecutionState(ES_CONTINUOUS | ES_SYSTEM_REQUIRED | ES_DISPLAY_REQUIRED)` and clears the request on exit. WGDot does not own or restore that worker. Do not alter the user's Windows power plan or claim Awtarchy's separate four-hour Hypridle safeguard exists on Windows.
...
- Use YASB's native `ControlCenterWidget` for Awtarchy-style quick settings. Keep brightness/volume/microphone/media on native YASB sections. Quick actions may invoke the standalone theme and coordinated auto-hide scripts, GlazeWM NoAlt/VM commands, Windows display settings, DND, and Windows dark mode. Because YASB v2.0.7 launches custom Control Center commands with `ShellExecute(..., SW_SHOWNORMAL)`, console-subsystem tools such as `glazewm.exe command` must be invoked in a way that does not flash a console window. Do not add a persistent helper or WGDot dependency merely to hide that window. Mouse mode stays out of quick settings; its direct binding is `Win+Alt+M`. Running applications stay visible and unshaded; do not expose the retired Running Apps/Shade Apps toggles. Remove YASB's screenshot quick action because Flameshot is the managed screenshot surface.
...
- Do not use `yasbc toggle-bar` for bar hiding: it can strand an invisible bar and does not coordinate the GlazeWM top gap. Managed config starts visible with YASB `auto_hide: false` and a 35 px GlazeWM top gap. `Alt+Ctrl+B` invokes the portable `~/.config/win-glaze/scripts/bar-autohide.ps1`, which coordinates YASB auto-hide with the 35 px/5 px GlazeWM gap and reloads the affected applications. Keep YASB `context_menu: false`; WGDot must not be required for bar auto-hide.
...
- Keep quick-settings theme launch and `Win+T` on the same Windows Terminal `theme-switcher.ps1` selector. Do not reintroduce a `wgdot theme` runtime command or WGDot-owned active-theme state.
- Launch the standalone theme selector in Windows Terminal titled `Win Glaze Themes` with `--suppressApplicationTitle`, and keep a matching GlazeWM `set-floating --centered` rule in both managed profiles. Theme application is owned by `~/.config/win-glaze/scripts/theme-switcher.ps1`, updates the tracked YASB theme file and Windows Terminal user settings, and must not reload GlazeWM.
...
- The visible GlazeWM binding-mode widget should reflect the native `noalt` / `vm` state. Mouse mode is currently omitted. Real pause is owned directly by GlazeWM through `wm-toggle-pause`; YASB must not poll WGDot pause state.
- Do not add AutoHotkey, whkd, DLL injection, or third-party general-purpose input daemons. There is no approved WGDot desktop worker in the current architecture. The native `mouse` binding mode itself requires no helper; only future actual mouse-button behavior would need a separately approved portable implementation. If clipboard history, lone-Super suppression, or similar custom interaction is reintroduced, implement it as a narrowly scoped portable dotfile-owned helper that can run without WGDot. Keep the Super+L hook test-only unless explicitly redesigned.
