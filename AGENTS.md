...
- Normal/personal mirrors Awtarchy's btop launcher on `Super+Shift+B` (both left/right Windows keys), including inside the Normal `noalt` mode because GlazeWM binding modes do not inherit global bindings. Launch upstream `btop4win.exe` in a dedicated Windows Terminal titled `btop` and float/center that terminal. Do not add this binding to Work while btop remains default OFF there.
