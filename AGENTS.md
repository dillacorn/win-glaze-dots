...
- Do not use `yasbc toggle-bar` as a keyboard hide shortcut: it can leave a user with an invisible bar and cannot reclaim WGDot's separate GlazeWM gap. Keep `auto_hide: false` explicit in managed config, but keep YASB's native blank-bar context menu enabled so a user can safely enable/disable its runtime auto-hide and recover the bar by hovering the edge. Upstream still exposes no supported CLI auto-hide toggle, so Awtarchy's keyboard auto-hide + dynamic 5/35 px GlazeWM gap behavior remains intentionally unimplemented until there is a state-safe mapping.
