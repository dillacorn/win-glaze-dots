...
- YASB theme and appearance files are tracked dotfiles, not WGDot-generated runtime state. Keep the Carbon Night palette in `styles.css` as a fallback, import tracked `theme.css` for palette variables, then import tracked `appearance.css` last. The compiled WGDot theme manager keeps its own lightweight state under `~/.config/win-glaze`, updates the managed/live `theme.css`, and synchronizes only its own Windows Terminal scheme/UI entries while preserving unrelated Terminal settings. Preserve the current Awtarchy palette names/colors unless intentionally changing the shared visual catalog. Never reload or restart GlazeWM for theme-only changes.
