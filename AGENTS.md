...
- WGDot runtime helpers are permitted for narrowly scoped custom primitives that native owners cannot provide cleanly, including coordinated YASB/GlazeWM auto-hide, idle inhibition through Windows execution-state APIs, live Awtarchy-style theme application/synchronization, the Awtarchy-style power surface, the Awtarchy-style application launcher, and the low-level mouse-button hook GlazeWM cannot express. Do not turn WGDot into a general desktop broker.
...
- Target Awtarchy-style clipboard/audio bindings without routing native-capable actions through synthetic WGDot hotkeys. Native Windows Clipboard History is working again on the gaming PC, but it still has no documented public API for opening/positioning the shell surface. Any clipboard-button positioning helper must therefore remain narrowly scoped and experimental until real Windows testing proves it reliable.
- Live audio ownership is GlazeWM -> EarTrumpet directly: global `Super+V` and `Alt+V` launch EarTrumpet's packaged AppsFolder identity, while `noalt` retains `Super+V` but deliberately leaves plain `Alt+V` uncaptured. WGDot's EarTrumpet packaged-setting integration may remain only as a management-time fallback/configuration path; desktop use must not route mixer activation through a WGDot synthetic-key helper.
...
- Launcher placement is trigger/context aware: a visible-bar click opens below/adjacent to the clicked application button; ordinary keyboard activation opens centered directly below the top bar; YASB auto-hide or a fullscreen/borderless foreground window forces center-screen placement on the relevant monitor. Keep the launcher single-instance and windowless at dispatch.
...
- Route quick-settings theme launch and `Super+Alt+T` through the approved compiled WGDot theme selector/manager in both profiles. Use the windowless frontend for non-interactive runtime actions, and use `wgdot.exe theme` inside Windows Terminal when an interactive selector is needed. Do not use WGDot for unrelated native actions.
