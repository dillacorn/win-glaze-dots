...
- Match Awtarchy clipboard/audio bindings: `Win+C` / `Super+C` calls WGDot's Clipboard History helper, while GlazeWM owns `Win+V` / `Super+V` for WGDot's EarTrumpet mixer helper. Both synthetic Windows/Alt helper paths must wait for the physical Windows modifier to be released before injecting the native shortcut.
