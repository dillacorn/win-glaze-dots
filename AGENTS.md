...
- EarTrumpet owns `Alt+V` through its own application hotkey setting. GlazeWM must not capture `Alt+V` or `Super+V`, and WGDot must not rewrite EarTrumpet's mixer hotkey during software reconciliation. YASB volume right-click may activate EarTrumpet directly through `explorer.exe shell:AppsFolder\40459File-New-Project.EarTrumpet_1sdd7yawvg6ne!EarTrumpet`; keep exactly one backslash after `AppsFolder` and do not route this through WGDot. Native Windows `Super+V` stays free for Clipboard History.
