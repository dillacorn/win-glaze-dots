...
- Match Awtarchy clipboard/audio bindings: `Win+C` / `Super+C` calls WGDot's Clipboard History helper, while GlazeWM owns `Win+V` / `Super+V` for WGDot's EarTrumpet mixer helper. Both synthetic Windows/Alt helper paths must wait for the physical Windows modifier to be released. Clipboard History must also preserve GlazeWM's existing pause state and briefly pause the WM while injecting native `Win+V`, because GlazeWM's low-level hook also sees injected key events and would otherwise route the helper back into the Super+V EarTrumpet binding. Do not use `NoWinKeys` for WGDot's optional shell-hotkey filter: it also disables native Win+V/Win+N surfaces these helpers need. Use the selective Explorer `DisabledHotkeys` path and preserve V/N.
