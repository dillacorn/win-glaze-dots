...
- The managed profiles include a `vm` binding mode toggled with `Win+Alt+V`. Ordinary Windows/Alt chords are intentionally not captured there so a focused guest can receive them. Host escape controls use `Win+Alt`; do not add a GlazeWM standalone-Windows-key consumer to VM mode. GlazeWM owns NoAlt/VM transitions directly with `wm-enable-binding-mode` / `wm-disable-binding-mode`; YASB's native `GlazewmBindingModeWidget` displays and clears the active mode. Do not reintroduce WGDot-tracked binding-mode state or route mode transitions through WGDot.
...
- GlazeWM 3.10.x does not expose mouse buttons through its keybinding parser. Any custom mouse-window mode must therefore be a portable dotfile-owned non-elevated helper, narrowly active only while that mode is enabled, with an unconditional escape path and no permanent background daemon. WGDot must not own the live mouse hook. Until a standalone implementation is present and maintainer-tested, do not expose a dead mouse-mode control.
...
- Use YASB's native `ControlCenterWidget` for Awtarchy-style quick settings. Keep brightness/volume/microphone/media on native YASB sections. Quick actions may invoke the standalone theme and coordinated auto-hide scripts, native GlazeWM NoAlt/VM commands, Windows display settings, DND, and Windows dark mode. Running applications stay visible and unshaded; do not expose the retired Running Apps/Shade Apps toggles. Mouse mode remains omitted until a portable implementation exists. Remove YASB's screenshot quick action because Flameshot is the managed screenshot surface.
