...
- Both GlazeWM profiles keep `cursor_jump.enabled: true` with `cursor_jump.trigger: "window_focus"` so pointer movement follows focused windows rather than only monitor focus.
