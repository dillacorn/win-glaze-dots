...
- The managed profiles include a `vm` binding mode toggled with `Win+Alt+V`. Ordinary Windows/Alt chords are intentionally not captured there so a focused guest can receive them. Host escape controls use `Win+Alt`; do not add the standalone-Windows-key consumer to VM mode. GlazeWM upstream stores at most one enabled binding mode because `wm-enable-binding-mode` replaces the active mode. Runtime testing on 2026-09-20/21 also showed `glazewm.exe query binding-modes` can fail to receive its IPC response while a large managed mode is active because the query serializes the full active mode config. Quick Settings mode toggles must therefore tolerate that query failure: retry, unconditionally disable the requested mode as a probe, then either leave it disabled when the query becomes empty or enable the requested mode to replace a different active mode.
...
- YASB Quick Launch is WGDot's primary Awtarchy-style application search, with installed applications as the default provider. Upstream YASB v2.0.7 implements widget hotkeys with Win32 `RegisterHotKey`, so a YASB-owned `Alt+P` or `Win+D` would remain global even while GlazeWM is in VM mode. Windows reserves `Win+Alt+D` for date/time, so YASB uses only the private `Win+Alt+F24` bridge. Normal/noalt GlazeWM own `Alt+P` and `Win+D` and route them through `wgdot quick-launch`, which waits for physical modifiers to release, preserves GlazeWM's pause state, pauses GlazeWM while injecting `Win+Alt+F24` with `SendInput`, then restores the prior pause state. Flow Launcher remains optional/default-off. WGDot's P/Invoke declaration must preserve the native Win32 `INPUT` union size (`40` bytes on x64, `28` on x86); include `MOUSEINPUT`, `KEYBDINPUT`, and `HARDWAREINPUT` in the union even though these bridges currently inject only keyboard input.
- Match Awtarchy clipboard/audio bindings: `Win+C` / `Super+C` calls WGDot's Clipboard History helper, while GlazeWM owns `Win+V` / `Super+V` for WGDot's EarTrumpet mixer helper. Both synthetic Windows/Alt helper paths must wait for the physical Windows modifier to be released. Clipboard History must also preserve GlazeWM's existing pause state and briefly pause the WM while injecting native `Win+V`, because GlazeWM's low-level hook also sees injected key events and would otherwise route the helper back into the Super+V EarTrumpet binding. Hidden `quick-launch` and `clipboard-history` helper failures must not disappear with the hidden GlazeWM/YASB process: WGDot records them under its local state and surfaces a Windows error dialog on failure. Do not use `NoWinKeys` for WGDot's optional shell-hotkey filter: it also disables native Win+V/Win+N surfaces these helpers need. Use the selective Explorer `DisabledHotkeys` path and preserve V/N.
...
- Windows RawAccel uses only `Win+Shift+M` / `Super+Shift+M` for its WGDot GUI toggle. Do not add the Awtarchy `Alt+Shift+M` alias on Windows; the user explicitly prefers the Super-only chord here. RawAccel should open floating and centered, and the launcher must eventually behave as a real show/hide toggle rather than blindly starting another process.
...
- Awtarchy's clipboard button maps to Windows Clipboard History through WGDot's helper and the bar's simple CustomWidget; keep Quick Launch's clipboard provider disabled so it does not duplicate that surface. Synthetic clipboard and launcher chords use `SendInput`. YASB callbacks that invoke WGDot use the generated GUI-subsystem `wgdotw.exe` wrapper so normal clicks do not flash a console window.
...
- Do not add AutoHotkey, whkd, DLL injection, or general-purpose background input daemons. Approved input-hook exceptions are narrowly scoped: WGDot's mouse hook may exist only for the lifetime of the `mouse` binding mode, and the Super+L keyboard hook may exist only when the maintainer explicitly starts `wgdot super-l-test`. Neither hook may become a silent startup/background service without a new explicit project decision.
