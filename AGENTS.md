...
- The workspace mover may use YASB's native Grouper + Applications widgets because GlazeWM already exposes native `move-workspace --direction` commands. Keep the documented difference that expansion is click-based rather than Awtarchy's hover drawer; current YASB Grouper exposes no hover-expand option.
