...
The managed `system-clipboard` module is loaded by `init.lua`; neither `Ctrl+C` nor `c y` may invoke it as a Yazi plugin action. Both bindings must run native Yazi `yank` first, then call `WgdotYaziMirrorYankToSystemClipboard()` as an inline Lua action. The mirror reads the already-completed `cx.yanked` set and starts `ya.async()` from that sync Lua context.
