...
- Windows reserves `Win+L` for session locking. Runtime testing confirmed it still locks under the validated NoWinKeys setup, and PowerToys documents it as non-remappable. Do not claim `Super+L` can provide GlazeWM focus-right under the current no-extra-hook project constraints; retain `Super+Right`/other supported focus-right bindings.
...
- Do not add AutoHotkey, whkd, keyboard hooks, DLL injection, or general-purpose background input daemons. The only approved input-hook exception is WGDot's scoped mouse-mode hook described above; it must exist only for the lifetime of the `mouse` binding mode.
