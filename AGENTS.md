...
The managed `system-clipboard` module is loaded by `init.lua`; neither `Ctrl+C` nor `c y` may invoke it as a Yazi plugin action. Both bindings call `WgdotYaziSystemYank()`, which arms a one-shot mirror and performs native Yazi `yank`. A local `@yank` subscription then mirrors that completed yank to the Windows FileDrop clipboard asynchronously.
