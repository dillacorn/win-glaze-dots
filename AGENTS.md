...
- Before any normal update/reset or dots-only apply that will write a managed `yazi-*` file, detect running `yazi.exe` processes. If Yazi is running, explain that its managed configuration is about to change and ask `Close Yazi and continue? [Y/n]`. Yes/default closes every Yazi process and verifies it exited before any managed file is written; No cancels the managed apply without partial writes. Keep the same guard in the PowerShell maintenance path.
