...
- The bar theme entrypoint uses native YASB `GrouperWidget` + `ApplicationsWidget` controls to expose direct `wgdot theme <id>` choices without opening a terminal. Keep `Win+T` as the interactive WGDot selector. Do not invent an active-theme indicator inside the Grouper unless YASB exposes a supported way to bind WGDot theme state into the widget.
