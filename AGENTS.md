...
- EarTrumpet owns `Alt+V` through its own application hotkey setting. GlazeWM and YASB must not launch EarTrumpet for `Alt+V`, `Super+V`, or volume right-click, and WGDot must not rewrite EarTrumpet's mixer hotkey during software reconciliation. This avoids the failed AppsFolder path that opened Explorer/Documents and keeps native Windows `Super+V` free for Clipboard History.
- Flameshot selection capture is `Win+Shift+X` in Normal/noalt and launches the WGDot-managed default installation path `"C:\Program Files\Flameshot\bin\flameshot.exe" gui` directly from GlazeWM; VM mode keeps its host-only `Win+Alt+S` escape binding. `Win+Shift+S` stays unbound by GlazeWM so the standard Windows Snipping Tool remains available. WGDot must not sit in the live screenshot path. The tracked Flameshot INI must contain no user-specific absolute profile paths or obsolete shortcut entries current Flameshot rejects, and it is intentionally non-merge-managed so dots-only/reset replaces stale invalid keys instead of preserving them.
...
- Keep native YASB volume left-click mute, middle alternate-label behavior, and wheel volume changes. Volume right-click is intentionally a no-op; EarTrumpet owns its own `Alt+V` hotkey and the rejected AppsFolder launch path must not return.
