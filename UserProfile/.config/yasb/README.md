...
- Awtarchy's workspace mover expands on hover and includes a mouse submap toggle. YASB's native Grouper expands by click and exposes no hover-expand option, while current GlazeWM does not expose mouse-button binding-mode inputs. WGDot therefore keeps the click-based directional mover and does not fake a mouse submap with an extra input hook.
