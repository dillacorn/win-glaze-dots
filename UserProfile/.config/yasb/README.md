...
| output audio | native Volume widget for mute/scroll volume with Awtarchy's `` muted glyph; right-click opens the existing WGDot EarTrumpet mixer helper, paralleling Awtarchy's Wiremix action | YASB Volume callbacks + existing WGDot `eartrumpet-mixer` integration |
