...
| CPU / memory | native CPU and Memory widgets, using the same unitless integer bar labels as current Awtarchy (`42 `, `63 `) | current Awtarchy `SystemState.cpuUsage` / `memoryUsage` are integers; YASB exposes the same percentage values without requiring a literal `%` suffix |
...
| battery | native Battery widget with `hide_unsupported`; the compact label matches Awtarchy's unitless percentage and retains the battery glyph while adding the charging bolt when plugged in. Left/right open Windows' own Power & battery settings, while middle-click toggles the alternate label | current Awtarchy renders `battery glyph + optional bolt + integer`; YASB v2.0.7 `charging_options.icon_format` natively supports both `{icon}` and `{charging_icon}`. YASB has no battery-details popup; its separate `PowerPlanWidget` cannot be composed into Battery's click surface |
...
| output audio | native Volume widget for mute/scroll volume with Awtarchy's `` muted glyph and matching low/medium/high icon cutoffs (`0-24`, `25-59`, `60-100`); right-click opens the existing WGDot EarTrumpet mixer helper, paralleling Awtarchy's Wiremix action | YASB v2.0.7 accepts numeric threshold keys in its native Volume icon map + existing WGDot `eartrumpet-mixer` integration |
...
| inline tray | native Systray widget with 14 px icons, no extra outer module padding, 10 px inter-icon spacing, and `use_hook: false` | current Awtarchy `TrayStrip` uses 14 px icons with 10 px row spacing and no enclosing BarButton; YASB v2.0.7 shares one native tray monitor service across all Systray widget instances while each instance keeps screen-specific state |
| clipboard history | a dedicated `QuickLaunchWidget` using Awtarchy's `` bar glyph with only YASB's native Windows Clipboard History provider enabled; it opens directly to history and supports text/images, restore, delete, clear, and previews | YASB Quick Launch and Clipboard History provider docs/source |
...
- Awtarchy's power button uses a distinct right-click open-only path. YASB v2.0.7 exposes `toggle_power_menu` but no separate open-only PowerMenu callback, so both Windows mouse buttons use the native toggle.
