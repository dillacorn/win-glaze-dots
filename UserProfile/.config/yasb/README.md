...
| visible Hyprland submap | `GlazewmBindingModeWidget` for WGDot's `noalt` / `vm` modes; both left and right click use YASB's native `disable_binding_mode`, matching Awtarchy's click-to-reset behavior | YASB v2.0.7 registers `disable_binding_mode` and sends the native GlazeWM disable command for the active mode |
...
| clipboard history | a dedicated `QuickLaunchWidget` using Awtarchy's actually rendered `` clipboard glyph with only YASB's native Windows Clipboard History provider enabled; it opens directly to history and supports text/images, restore, delete, clear, and previews | current Awtarchy `Bar.qml` supplies the legacy `` token, while `BarButton.qml` translates that token to `` before rendering; YASB Quick Launch provides the native Windows Clipboard History surface |
