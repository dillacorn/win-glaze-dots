...
| clipboard history | the `` bar button and `Super+C` call WGDot's Clipboard History helper; the helper currently synthesizes Windows' native `Win+V` chord | GlazeWM no longer binds Win+V to EarTrumpet. During the experimental NoWinKeys test, whether injected Win+V still opens Clipboard History is intentionally runtime-tested rather than assumed |
