...
Assert-NotContains $config "EarTrumpet_1sdd7yawvg6ne!EarTrumpet" "YASB no longer tries to launch EarTrumpet; its own Alt+V hotkey owns mixer activation"
...
foreach ($glaze in @($glazeNormal, $glazeWork)) {
    ...
    Assert-Contains $glaze "bindings: [`"lwin+shift+x`", `"rwin+shift+x`"]" "Super+Shift+X launches Flameshot"
    Assert-NotContains $glaze "bindings: [`"lwin+shift+s`", `"rwin+shift+s`"]" "Super+Shift+S remains native Windows Snipping Tool"
    Assert-Contains $glaze "C:\\Program Files\\Flameshot\\bin\\flameshot.exe" "Flameshot uses its installed path instead of GlazeWM PATH lookup"
