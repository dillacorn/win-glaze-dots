...
Assert-Contains $config "workspace_move_hub:" "YASB keeps a passive hub for the workspace mover"
Assert-Contains $config "yasb.custom.CustomWidget" "workspace mover hub uses a native passive YASB widget"
Assert-Contains $config "class_name: `"workspace-move-hub`"" "workspace mover hub has its dedicated styling class"
...
Assert-Contains $wifiBlock "on_left: `"toggle_menu`"" "Wi-Fi/Ethernet icon uses YASB's native toggleable network menu"
Assert-NotContains $wifiBlock "ms-settings:network-status" "Wi-Fi/Ethernet icon no longer repeatedly opens Windows Network settings"
Assert-Contains $bluetoothBlock "on_left: `"toggle_menu`"" "Bluetooth icon uses YASB's native toggleable Bluetooth menu"
Assert-NotContains $bluetoothBlock "ms-settings:bluetooth" "Bluetooth icon no longer repeatedly opens Windows Bluetooth settings"
...
Assert-Contains $clipboardBlock "wgdotw.exe clipboard-anchor bar" "clipboard button uses the narrow native Clipboard History anchor"
...
foreach ($glaze in @($glazeNormal, $glazeWork)) {
    ...
    Assert-Contains $glaze "bindings: [`"lwin+c`", `"rwin+c`"]" "GlazeWM keeps Super+C for native Clipboard History"
    Assert-Contains $glaze "wgdotw.exe clipboard-anchor hotkey" "Super+C uses the narrow native Clipboard History anchor"
...
Assert-Contains -Text $style -Needle ".workspace-move-hub" -Message "workspace mover remains an always-visible hover hub"
...
Assert-Contains -Text $readme -Needle "wgdotw.exe clipboard-anchor" -Message "native Clipboard History anchor and Super+C ownership are documented"
