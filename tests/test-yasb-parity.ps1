...
Assert-Contains $config 'keys: "win+alt+f24"' "YASB Quick Launch uses the private non-reserved bridge"
Assert-NotContains $config 'keys: "win+alt+d"' "YASB does not collide with Windows' Win+Alt+D date/time shortcut"
...
Assert-Contains $config 'command: "wgdotw theme-toggle"' "quick settings exposes themes without a console flash"
Assert-Contains $config 'command: "wgdotw bar-autohide-toggle"' "quick settings exposes auto-hide without a console flash"
Assert-Contains $config 'command: "wgdotw glazewm-binding-mode-toggle noalt"' "quick settings toggles NoAlt mode"
Assert-Contains $config 'command: "wgdotw glazewm-binding-mode-toggle vm"' "quick settings toggles VM mode"
Assert-Contains $config 'exec wgdotw glazewm-binding-mode-set normal' "visible binding-mode label clears modes through WGDot tracked state"
Assert-Contains $config 'exec wgdotw glazewm-pause-toggle' "pause clicks use the GUI-subsystem WGDot wrapper"
...
Assert-Contains $clipboardBlock 'exec wgdotw clipboard-history' "clipboard bar button opens WGDot Clipboard History"
...
Assert-Contains $native 'if (command == "clipboard-history") return OpenWindowsClipboardHistory();' "WGDot exposes its native Clipboard History surface"
