...
Assert-NotContains $config "mouse-mode-toggle" "passive mouse icon has no retired mouse-mode helper dependency"
...
Assert-Contains $config "-WindowStyle Hidden" "console-subsystem quick actions use hidden PowerShell rather than flashing a console"
...
Assert-Contains $config "wm-enable-binding-mode --name noalt" "quick settings enters NoAlt through GlazeWM"
Assert-Contains $config "wm-enable-binding-mode --name vm" "quick settings enters VM through GlazeWM"
...
Assert-Contains $config "idle-inhibitor.ps1" "idle inhibitor polls the standalone script"
...
Assert-Contains $config "toggle`"" "idle inhibitor callbacks toggle the standalone Keep Awake script"
