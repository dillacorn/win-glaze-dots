...
Assert-Contains $readme 'theme-switcher.ps1' "standalone theme switching is documented"
Assert-Contains $readme 'bar-autohide.ps1' "standalone coordinated auto-hide is documented"
Assert-Contains $readme 'idle-inhibitor.ps1' "standalone idle inhibitor is documented"
Assert-Contains $readme 'PowerMenuWidget' "native YASB power ownership is documented"
Assert-Contains $readme 'EarTrumpet owns the chord itself' "EarTrumpet direct Super+V ownership is documented"
Assert-Contains $readme 'no managed `Super+C` custom history shortcut' "clipboard omission is explicit instead of claiming a dead helper"
Assert-Contains $readme 'Mouse-window mode is currently omitted' "mouse-mode omission is explicit"
Assert-Contains $readme 'CPU temperature' "CPU temperature is not silently substituted with another metric"
Assert-Contains $readme 'Do Not Disturb' "notification/DND mapping is documented"
...
Assert-Contains $readme 'conditional primary-label expression' "Bluetooth connected-name/count limitation is documented"
Assert-Contains $readme 'no bar-label connectivity CSS class' "network disconnected-color limitation is documented"
Assert-Contains $readme '#a1a1a1' "neutral GlazeWM border rationale is documented"
Assert-Contains $readme 'does **not** reload GlazeWM' "theme changes remain reload-free"
