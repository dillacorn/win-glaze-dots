...
Assert-True ($yaziKeymapText -match 'on = \["g", "r"\].*plugin recent-files') "Yazi g r opens recently opened files"
Assert-True ($yaziKeymapText -match 'on = \["g", "r"\].*Go to recently opened folder') "Yazi g r help describes the virtual recent-files folder"
...
Assert-True ($yaziKeymapText -match 'on = \["g", "b"\].*plugin bookmarks') "Yazi g b opens bookmarked folders"
Assert-True ($yaziKeymapText -match 'on = \["g", "B"\].*WgdotYaziToggleBookmark') "Yazi g B toggles current-directory bookmarks"
Assert-True ($yaziKeymapText -match 'desc = "Bookmark current directory"') "Yazi g B documents current-directory bookmark semantics"
...
Assert-True ($yaziInitText -match 'ya\.sleep\(25\)') "Yazi preview cache refit is deferred until after the first pane resize render"
...
Assert-True ($yaziInitText -match 'x = area\.x \+ area\.w - 3') "Yazi preview visibility button sits at the current-pane right edge"
...
Assert-True ($yaziInitText -match 'local label = visible and " 󰞔 " or " 󰞓 "') "Yazi preview visibility glyphs are correct"
Assert-True ($yaziInitText -match ':type\(ui\.Border\.PLAIN\)') "Yazi custom context menu uses square corners"
Assert-True ($yaziInitText -match 'function WgdotYaziToggleBookmark\(\)') "Yazi bookmark toggle helper is defined"
Assert-True ($yaziInitText -match 'WgdotYaziBookmarkTarget\(tostring\(cx\.active\.current\.cwd\), true\)') "Yazi g B bookmarks the current directory"
...
Assert-True ($yaziBookmarksText -match 'local ROOT = "wgdot-bookmarks://collection//"') "Yazi bookmarks expose a flat scope VFS collection root"
Assert-True ($yaziBookmarksText -match 'function M:ReadDir\(job\)') "Yazi bookmarks implement the stable VFS directory provider"
Assert-True ($yaziBookmarksText -match 'function M:Trash\(job\)') "Yazi deleting a bookmark row removes only the virtual entry"
...
Assert-True ($yaziInitText -match 'and " 󰘕 " or " 󰹶 "') "Yazi preview control uses the compact restore/maximize Nerd Font glyphs"
Assert-True ($yaziInitText -match 'w = 3') "Yazi preview control keeps a compact three-cell click target"
...
Assert-True ($yaziRecentText -match 'local ROOT = "wgdot-recents://collection//"') "Yazi recents expose a flat scope VFS collection root"
Assert-True ($yaziRecentText -match 'function M:ReadDir\(job\)') "Yazi recents implement the stable VFS directory provider"
Assert-True ($yaziRecentText -match 'function M:Trash\(job\)') "Yazi deleting a recent row removes only the virtual history entry"
Assert-True ($yaziRecentText -match 'ya\.emit\("cd", \{ Url\(ROOT\) \}\)') "Yazi g r enters the recents VFS directory"
...
Assert-True ($yaziBookmarksText -match 'ya\.emit\("cd", \{ Url\(ROOT\) \}\)') "Yazi g b enters the bookmarks VFS directory"
Assert-True ($yaziVfsText -match '\[wgdot-bookmarks\."\*"\]') "Yazi bookmark VFS service is configured"
Assert-True ($yaziVfsText -match '\[wgdot-bookmarks\."\*"\][\s\S]*?kind\s*=\s*"scope"') "Yazi bookmark VFS uses flat scope semantics"
Assert-True ($yaziVfsText -match '\[wgdot-recents\."\*"\]') "Yazi recents VFS service is configured"
Assert-True ($yaziVfsText -match '\[wgdot-recents\."\*"\][\s\S]*?kind\s*=\s*"scope"') "Yazi recents VFS uses flat scope semantics"
Assert-True ($yaziVfsText -match 'run\s*=\s*"bookmarks"') "Yazi bookmark VFS routes to the managed plugin"
Assert-True ($yaziVfsText -match 'run\s*=\s*"recent-files"') "Yazi recents VFS routes to the managed plugin"
...
Assert-True ($nativeSourceText -match 'const string Version = "native-preview-83"') "native runtime version tracks current WGDot maintenance changes"
