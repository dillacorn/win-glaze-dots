...
Assert-True ($yaziKeymapText -match 'on = \["<C-c>"\].*\["yank".*WgdotYaziMirrorYankToSystemClipboard') "Yazi Ctrl+C runs native yank before the system clipboard mirror"
Assert-True ($yaziKeymapText -notmatch 'on = \["<C-c>"\].*plugin system-clipboard') "Yazi Ctrl+C never enters a plugin action"
Assert-True ($yaziKeymapText -match 'on = \["c", "y"\].*\["yank".*WgdotYaziMirrorYankToSystemClipboard') "Yazi c y runs native yank before the same system clipboard mirror"
Assert-True ($yaziKeymapText -notmatch 'on = \["c", "y"\].*plugin system-clipboard') "Yazi c y never enters a plugin action"
...
Assert-True ($yaziInitText -match 'require\("system-clipboard"\)') "Yazi init loads the managed Windows file-clipboard module"
Assert-True ($yaziInitText -match 'function WgdotYaziMirrorYankToSystemClipboard\(\)') "Yazi exposes a post-yank Windows clipboard mirror"
$yaziClipboardMirrorBlock = [regex]::Match($yaziInitText, '(?ms)function WgdotYaziMirrorYankToSystemClipboard\(\).*?^end').Value
Assert-True ($yaziClipboardMirrorBlock -match 'pairs\(cx\.yanked\)') "Yazi Windows clipboard mirror reads the completed native yank set"
Assert-True ($yaziClipboardMirrorBlock -match 'ya\.async\(function\(\)') "Yazi Windows clipboard mirror starts async work from the sync inline-Lua action"
Assert-True ($yaziInitText -notmatch 'ps\.sub(?:_remote)?\("@yank"') "Yazi Windows clipboard mirror does not depend on DDS yank event payloads"
