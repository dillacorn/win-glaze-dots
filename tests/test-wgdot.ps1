...
Assert-True ($yaziInitText -match 'WgdotYaziContextMenu:show\("item", event\.x, event\.y, selected_count\)') "Yazi item right-click opens the proven custom Lua context menu"
...
Assert-True ($yaziInitText -match 'Modal:children_add\(WgdotYaziContextMenu, 20\)') "Yazi right-click menu remains a custom Lua Modal child"
Assert-True ($yaziInitText -match 'function WgdotYaziContextMenu:click\(event, up\)') "Yazi custom context menu handles direct row clicks"
Assert-True ($yaziInitText -match 'local row = event\.y - self\._list_area\.y \+ 1') "Yazi custom context menu uses the proven direct row hit test"
Assert-True ($yaziInitText -match 'local action = self:actions\(\)\[row\]') "Yazi custom context menu directly maps rows to actions"
Assert-True ($yaziInitText -match 'self:run\(action\.action\)') "Yazi custom context menu directly dispatches clicked actions"
Assert-True ($yaziInitText -match 'function WgdotYaziContextMenu:footer\(\)') "Yazi custom context menu keeps the proven footer layout"
Assert-True ($yaziInitText -match 'local width = math\.min\(56, area\.w\)') "Yazi custom context menu keeps the proven stable width"
Assert-True ($yaziInitText -match 'local height = math\.min\(#actions \+ 4, area\.h\)') "Yazi custom context menu keeps the proven action-plus-footer height"
...
Assert-True ($yaziInitText -notmatch '"wgdot-context-menu"|WgdotYaziPluginArgs\("show", values\)') "Yazi right-click menu has no async Which dependency"
