...
Assert-True ($nativeSourceText -match 'const string Version = "native-preview-42"') "native runtime version tracks live YASB theme support"
...
foreach ($text in @($glazeNormalText, $glazeWorkText)) {
    ...
    Assert-True (($text -split 'bindings:\s*\["lwin\+t",\s*"rwin\+t"\]').Count - 1 -ge 2) "Win+T theme picker exists globally and in noalt"
    Assert-True ($text -match 'shell-exec wt\.exe -w new --size 72,22 nt --title "WGDot Themes" --suppressApplicationTitle wgdot theme') "theme shortcut opens stable titled WGDot selector"
