...
Assert-True ($yaziKeymapText -match 'on = \["<Space>"\].*WgdotYaziToggleOrCommitSelection') "Yazi Space toggles or commits range selection"
Assert-True ($yaziKeymapText -match 'on = \["<C-Space>"\].*run = "toggle"') "Yazi Ctrl+Space toggles individual selection"
Assert-True ($yaziKeymapText -match 'on = \["<S-Up>"\].*WgdotYaziEnsureRangeSelect.*arrow prev') "Yazi Shift+Up extends range selection"
Assert-True ($yaziKeymapText -match 'on = \["<S-Down>"\].*WgdotYaziEnsureRangeSelect.*arrow next') "Yazi Shift+Down extends range selection"
