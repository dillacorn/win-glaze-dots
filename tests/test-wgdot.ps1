...
Assert-True ($yaziKeymapText -match 'on = \["<C-c>"\].*run = "yank"') "Yazi Ctrl+C is native yank only"
Assert-True ($yaziKeymapText -notmatch 'on = \["<C-c>"\].*plugin system-clipboard|WgdotYaziMirrorYankToSystemClipboard') "Yazi Ctrl+C never enters Windows clipboard integration"
Assert-True ($yaziKeymapText -match 'on = \["c", "y"\].*\["yank".*"plugin system-clipboard"') "Yazi c y explicitly yanks then exports to the Windows file clipboard"
