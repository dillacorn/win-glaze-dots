...
Assert-True ($yaziKeymapText -match 'on = \["<C-c>"\].*run = "yank"') "Yazi Ctrl+C uses native yank only"
Assert-True ($yaziKeymapText -notmatch 'on = \["<C-c>"\].*plugin system-clipboard') "Yazi Ctrl+C does not invoke the Windows clipboard plugin"
Assert-True ($yaziKeymapText -match 'on = \["c", "y"\].*"yank".*"plugin system-clipboard"') "Yazi c y keeps explicit Windows file-clipboard export separate from Ctrl+C"
