...
Assert-True ($yaziKeymapText -match 'on = \["<Delete>"\].*run = "remove"') "Yazi Delete sends selected items to trash"
...
Assert-True ($yaziInitText -match 'WgdotYaziBookmarkTarget\(tostring\(cx\.active\.current\.cwd\)\)') "Yazi g B bookmarks the current directory"
Assert-True ($yaziInitText -match 'WgdotYaziBookmarkTarget\(tostring\(hovered\.url\)\)') "Yazi context menu can bookmark a precise hovered file/folder"
...
Assert-True ($yaziInitText -match 'ya\.emit\("peek", \{ force = true \}\)') "Yazi preview ratio changes force a stable re-peek"
Assert-True ($yaziInitText -match 'ui\.render\(\)') "Yazi time toggle requests an immediate redraw"
Assert-True ($yaziInitText -match 'Entity:children_add\(function\(\)') "Yazi rows include subtle leading spacing"
...
Assert-True ($yaziBookmarksText -match 'local path = decode_arg\(job\.args\[2\]\)') "Yazi bookmarks decode managed path arguments"
...
Assert-True ($yaziInitText -match 'WgdotYaziBookmarkTarget\(tostring\(cx\.active\.current\.cwd\)\)') "Yazi g B bookmarks the current directory"
Assert-True ($yaziInitText -match 'WgdotYaziBookmarkTarget\(tostring\(hovered\.url\)\)') "Yazi item context menu bookmarks the hovered file or folder"
Assert-True ($yaziBookmarksText -match 'content = "No bookmarked items\."') "Yazi bookmarks expose a multi-item picker"
Assert-True ($yaziBookmarksText -match 'ya\.emit\("reveal", \{ Url\(item\.path\), raw = true \}\)') "Yazi file bookmarks reveal the bookmarked file"
...
Assert-True ($yaziRecentText -match 'local MAX_RECENTS = 35') "Yazi recents picker has a bounded retained history"
...
Assert-True ($yaziRecentText -match 'content = "No recently opened files\."') "Yazi empty recents message is concise"
Assert-True ($yaziRecentText -match 'cha and not cha\.is_dir') "Yazi recents picker shows files only"
Assert-True ($yaziRecentText -match 'ya\.which') "Yazi g r uses the native Which picker"
Assert-True ($yaziRecentText -match 'ya\.emit\("reveal"') "Choosing a recent file reveals it in Yazi instead of launching it"
