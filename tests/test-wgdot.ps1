...
Assert-True ($yaziBookmarksText -match 'local ROOT = "wgdot-bookmarks://collection/"') "Yazi bookmarks expose a VFS collection"
...
Assert-True ($yaziRecentText -match 'local ROOT = "wgdot-recents://collection/"') "Yazi recents expose a VFS collection"
...
Assert-True ($yaziInitText -match 'function WgdotYaziRemoveMenu\(\)') "Yazi managed delete chooser is defined"
Assert-True ($yaziInitText -match 'Permanently delete\.\.\.') "Yazi delete chooser exposes permanent deletion"
