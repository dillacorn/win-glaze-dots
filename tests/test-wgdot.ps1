...
Assert-True ($nativeSourceText -match 'const string Version = "native-preview-51"') "native runtime version tracks current WGDot maintenance changes"
...
Assert-True ($nativeSourceText -match '(?s)static int OpenYasbQuickLaunch\(\).*?bool wasPaused = GlazeWmIsPaused\(\);.*?finally.*?if \(!wasPaused\).*?GlazeWmPauseToggle\(\);') "YASB Quick Launch bridge preserves GlazeWM pause state around injected Win+Alt+D"
...
Assert-True ($nativeSourceText -notmatch 'String\.Equals\(command, "mouse-mode-toggle"') "mouse-mode hotkey skips remote runtime-refresh checks for immediate response"
