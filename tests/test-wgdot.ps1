...
Assert-True ($nativeSourceText -match 'const string Version = "native-preview-58"') "native runtime version tracks current WGDot maintenance changes"
...
Assert-True ($nativeSourceText -match '(?s)static int OpenYasbQuickLaunch\(\).*?SendKeyChord\(VkLwin, VkMenu, VkF24\).*?finally.*?if \(!wasPaused\).*?GlazeWmPauseToggle\(\);') "YASB Quick Launch bridge preserves pause state around private Win+Alt+F24 SendInput"
