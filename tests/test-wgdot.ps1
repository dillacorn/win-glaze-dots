...
Assert-True ($yaziKeymapText -match 'on = \["B"\].*WgdotYaziBookmarkHovered') "Yazi Shift+B bookmarks the highlighted item"
Assert-True ($yaziKeymapText -match 'on = \["g", "b"\].*WgdotYaziGoBookmarks') "Yazi g b explicitly opens bookmarked folders"
Assert-True ($yaziKeymapText -notmatch 'on = \["g", "B"\]') "Yazi no longer overloads the Go namespace with bookmark mutation"
...
Assert-True ($yaziKeymapText -match 'on = \["R"\].*run = "rename"') "Yazi Shift+R renames the highlighted file or folder"
...
Assert-True ($yaziInitText -match 'label = "Rename".*shortcut = "R"') "Yazi item context menu shows the Shift+R rename shortcut"
