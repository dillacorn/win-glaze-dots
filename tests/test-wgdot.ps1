...
Assert-True ($yaziKeymapText -match 'on = \["g", "B"\].*WgdotYaziToggleBookmark') "Yazi g B toggles hovered/current folder bookmarks"
...
Assert-True ($yaziInitText -notmatch 'tostring\(file\.path\)') "Yazi recents do not use the nonexistent File.path field"
...
Assert-True ($yaziInitText -match 'ya\.emit\("plugin", \{ "fd" \}\)') "Yazi name search uses native fd plugin"
Assert-True ($yaziInitText -match 'ya\.emit\("plugin", \{ "rg" \}\)') "Yazi content search uses native rg plugin"
