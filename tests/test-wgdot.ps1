...
Assert-True ($yaziKeymapText -match 'on = \["<C-c>"\].*WgdotYaziSystemYank') "Yazi Ctrl+C uses the native-yank system clipboard wrapper"
Assert-True ($yaziKeymapText -notmatch 'on = \["<C-c>"\].*plugin system-clipboard') "Yazi Ctrl+C never enters a plugin action"
Assert-True ($yaziKeymapText -match 'on = \["c", "y"\].*WgdotYaziSystemYank') "Yazi c y aliases the same native-yank system clipboard wrapper"
...
Assert-True ($yaziInitText -match 'ps\.sub\("@yank"') "Yazi Windows clipboard mirror runs from the completed native yank event"
Assert-True ($yaziInitText -match 'function WgdotYaziSystemYank\(\)') "Yazi exposes a native-yank wrapper for Ctrl+C and c y"
Assert-True ($yaziInitText -match 'WgdotYaziSystemClipboardArmed = true\s+ya\.emit\("yank", \{\}\)') "Yazi system-copy wrapper arms the mirror before native yank"
